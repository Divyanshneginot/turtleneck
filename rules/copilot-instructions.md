# GitHub Copilot Workspace Instructions — Turtleneck Mode

When generating or refactoring UI components or styling, enforce Turtleneck mode.
...
* **Non-negotiables** (apply to EVERY surface): User intent and supplied constraints (never overwrite supplied brand/tokens); semantic elements (`button`, `input`, `dialog`); complete interaction states; accessible contrast (>=4.5:1 text, >=3:1 boundaries/focus); every `<input>` and icon-only `<button>` carries an `aria-label`; compositor-only transitions (`transform`, `opacity`) — never `transition: all`; `prefers-reduced-motion: reduce` fallback on every page; honest evidence (never fabricate metrics or unverified certifications); never leak skill-internal vocabulary ("Atelier", "Starlight", "Vernacular Harmony", "frequency-gated", "anti-slop") into user-facing output.
...
**Phase 3 — Blueprint Alignment.** Lock the data schema and tokens, then run the anti-slop gate.
Reject AI slop:

* NO saturated purple/cyan blurred nebula blobs.
* NO glassmorphism under 80% fill opacity, or with text contrast below 4.5:1.
* NO hollow bento cards, decorative 3D spheres, or buzzword copy.

**Phase 4 — Production Build.** Production standards:

* 5 states for all clickable elements: `default`, `hover`, `active` (`scale(0.98)`),
  `focus-visible` (2px ring, 3:1, 2px offset), `disabled`.
